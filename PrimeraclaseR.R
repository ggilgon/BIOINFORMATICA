...
# Creamos la lista, siemre y cuando pongamos la funcion c, Crearlos con la función c(): 
x<-c(2,0,0,4)
y<-c(1,9,9,9)
...
x[1]
x[-1] (todos los numeros menos el de la primera posicion, sin embargo en python solo te salia el ultimo)
...
str(iris)
summary(iris)
dim(iris) #para ver las dimensiones de la tabla
iris$Species
...
genesets<-vector("list",3) (va a ser una lista, con tres posiciones)
genesets[[1]]<-geneset1
genesets[[2]]<-geneset2
genesets[[3]]<-geneset3
genesets
...
apply(mi_matriz,1,mean) #aplicame la media de todas las filas
apply(mi_matriz,2,sum) #suma de todas las columnas
